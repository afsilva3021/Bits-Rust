...
const librarySearchControl = document.getElementById('librarySearchControl');
...
const gameSearchModal = gameSearchModalElement
    ? (hasBootstrap ? new bootstrap.Modal(gameSearchModalElement) : createModalFallback(gameSearchModalElement))
    : null;
const gameSearchModalInput = document.getElementById('gameSearchModalInput');
const gameSearchKeyboard = document.getElementById('gameSearchKeyboard');
const closeGameSearchModal = document.getElementById('closeGameSearchModal');
const applyGameSearchModal = document.getElementById('applyGameSearchModal');
...
const profileSourceModal = profileSourceModalElement
    ...
    : null;
const controllerAccessModalElement = document.getElementById('controllerAccessModal');
const controllerAccessModal = controllerAccessModalElement
    ? (hasBootstrap ? new bootstrap.Modal(controllerAccessModalElement) : createModalFallback(controllerAccessModalElement))
    : null;
const controllerAccessList = document.getElementById('controllerAccessList');
const controllerAccessConfirm = document.getElementById('controllerAccessConfirm');
const controllerAccessCancel = document.getElementById('controllerAccessCancel');
const controllerClosePageModalElement = document.getElementById('controllerClosePageModal');
const controllerClosePageModal = controllerClosePageModalElement
    ? (hasBootstrap ? new bootstrap.Modal(controllerClosePageModalElement) : createModalFallback(controllerClosePageModalElement))
    : null;
const controllerClosePageList = document.getElementById('controllerClosePageList');
const controllerClosePageConfirm = document.getElementById('controllerClosePageConfirm');
const controllerClosePageCancel = document.getElementById('controllerClosePageCancel');
...
let controllerBrowserTabs = [];
let selectedControllerPageIndex = 0;
function finishIntro() {
    ...
    enterLauncherOrShowProfileSelection();
    window.setTimeout(() => {
...
async function enterLauncherOrShowProfileSelection() {
    if (sessionStorage.getItem('gp-profile-selected') === '1') {
...
if (addLutrisForm) {
    addLutrisForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await window.pywebview.api.adicionar_jogo_lutris(lutrisGameName.value, lutrisGameId.value, lutrisGameCategory?.value || 'Lutris');
            if (!response.ok) {
...
if (importSteamLibraryButton) {
    importSteamLibraryButton.addEventListener('click', async () => {
        ...
        try {
            const response = await withUiTimeout(window.pywebview.api.importar_biblioteca_steam(), 90000, 'A importação da Steam demorou demais. Verifique conexão, Steam API Key e tente novamente.');
            if (!response.ok) {
                showControllerAlert(response.message);
                return;
            }
...
gameSearchModalElement?.addEventListener('hidden.bs.modal', () => {
    resetLauncherControllerInput(500);
    window.setTimeout(() => setNavigationArea('search', 0, false), 50);
});
gameSearchModalElement?.addEventListener('shown.bs.modal', () => {
    resetLauncherControllerInput(500);
    useVirtualKeyboard('search');
    setActiveKeyboardInput(gameSearchModalInput);
...
gameSearchModalInput?.addEventListener('focus', () => {
    setActiveKeyboardInput(gameSearchModalInput);
});
...
function getNavbarItems() {
    return Array.from(new Set(document.querySelectorAll('.navbar-toggler, .navbar-brand, #themeToggle, .app-action-btn, .steam-add-button')));
}
...
class GameSearchController {
    constructor(options) {
        ...
        this.modalInput = options.modalInput;
    ...
    bindEvents() {
        this.input?.addEventListener('input', () => {
            this.apply(this.input.value, false);
        });
        ...
        this.input?.addEventListener('keydown', (event) => {
            if (event.key === 'Enter') {
                event.preventDefault();
                this.apply(this.input.value, true);
            }
        });
        this.control?.addEventListener('click', (event) => {
            this.onFocus?.();
            if (event.target === this.clearButton || this.clearButton?.contains(event.target)) {
                return;
            }
        ...
        this.modalInput?.addEventListener('input', () => {
            this.apply(this.modalInput.value, false);
        });
        this.closeButton?.addEventListener('click', () => {
            this.modal?.hide();
        });
        this.applyButton?.addEventListener('click', () => {
            this.apply(this.modalInput?.value || '', true);
            this.modal?.hide();
        });
    }
    getItems() {
    ...
    compileRegex() {
        const pattern = String(this.pattern || '').trim();
        if (!pattern) {
        ...
        try {
            ...
            return { regex: null, error: `Regex invalida: ${error.message}` };
        }
    }
    updateStatus(error = '') {
        if (!this.status) {
        ...
        this.status.classList.toggle('d-none', !error);
        this.control?.classList.toggle('is-invalid', Boolean(error));
    }
    buildSearchText(game, card) {
        return [
            game?.nome,
            game?.title,
            game?.categoria,
            ...
            game?.caminho,
            game?.path,
            card?.dataset?.gameSearchText
        ].filter(Boolean).join(' ');
    }
    syncInputs(value = this.pattern) {
        if (this.input && this.input.value !== value) {
            this.input.value = value;
        }
        if (this.modalInput && this.modalInput.value !== value) {
            this.modalInput.value = value;
        }
    }
    apply(value = this.input?.value || '', scrollIntoView = false) {
        this.pattern = String(value || '');
        this.syncInputs();
        this.onApply?.(scrollIntoView);
    }
}
const gameSearchController = new GameSearchController({
    control: librarySearchControl,
    input: gameSearchInput,
    clearButton: clearGameSearch,
    status: gameSearchStatus,
    modal: gameSearchModal,
    modalInput: gameSearchModalInput,
    closeButton: closeGameSearchModal,
    applyButton: applyGameSearchModal,
    onApply: (scrollIntoView) => applyCategoryFilter(activeCategoryFilter, scrollIntoView),
    onFocus: () => setNavigationArea('search', 0, false)
});
gameSearchController.bindEvents();
function getSearchItems() {
    return gameSearchController.getItems();
}
function compileGameSearchRegex() {
    return gameSearchController.compileRegex();
}
function getGameSearchText(game, card) {
    return gameSearchController.buildSearchText(game, card);
}
function syncGameSearchInputs(value) {
    gameSearchController.syncInputs(value);
}
function applyGameSearch(value = gameSearchInput?.value || '', scrollIntoView = false) {
    gameSearchController.apply(value, scrollIntoView);
}
function applyCategoryFilter(category = activeCategoryFilter, scrollIntoView = false) {
    ...
    const { regex, error } = compileGameSearchRegex();
    gameSearchController.updateStatus(error);
    ...
    document.querySelectorAll('[data-game-column-id]').forEach((column) => {
        ...
        const searchText = card?.dataset.gameSearchText || '';
        let visibleByCategory = false;
        if (activeCategoryFilter === 'Retro') {
            visibleByCategory = gameType === 'rom';
        }
        else if (activeCategoryFilter === 'Lutris') {
            visibleByCategory = gameType === 'lutris';
        }
        else if (activeCategoryFilter === 'Todos') {
            visibleByCategory = gameType === 'steam' || gameType === 'executavel' || gameType === 'lutris';
        }
        else if (activeCategoryFilter === 'Linux') {
            visibleByCategory = gameType === 'steam' && isLinuxCompatibleTier(card?.dataset.protonTier);
        }
        else {
            visibleByCategory =
                gameType !== 'rom' &&
        ...
        const visibleBySearch = error ? false : (!regex || regex.test(searchText));
        const visible = visibleByCategory && visibleBySearch;
...
function updateNavigationSelection(scrollIntoView = true) {
    ...
    if (scrollIntoView && selectedItem) {
        ...
        else if (selectedItem.closest('.library-actions')) {
            scrollLibraryHeaderIntoView();
        }
...
function updateLibraryCount() {
    ...
    const searchLabel = gameSearchController.pattern.trim() ? ' encontrados' : '';
    if (libraryGameCount) {
        libraryGameCount.textContent = activeCategoryFilter === 'Todos'
            ? `${label}${searchLabel}`
            : `${label}${searchLabel} em ${activeCategoryFilter}`;
    }
}
...
function isControllerAccessModalOpen() {
    return controllerAccessModalElement?.classList.contains('show');
}
function isControllerClosePageModalOpen() {
    return controllerClosePageModalElement?.classList.contains('show');
}
function getControllerPageOptions(container) {
    return Array.from(container?.querySelectorAll('.controller-page-option:not([disabled])') || []);
}
function updateControllerPageSelection(container, scrollIntoView = true) {
    const options = getControllerPageOptions(container);
    if (!options.length) {
        return;
    }
    selectedControllerPageIndex = Math.max(0, Math.min(options.length - 1, selectedControllerPageIndex));
    options.forEach((option, index) => {
        const isSelected = index === selectedControllerPageIndex;
        option.classList.toggle('is-controller-selected', isSelected);
        option.setAttribute('tabindex', isSelected ? '0' : '-1');
    });
    const selectedOption = options[selectedControllerPageIndex];
    selectedOption.focus({ preventScroll: true });
    if (scrollIntoView) {
        selectedOption.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
    }
}
function navigateControllerPageModal(direction) {
    const container = isControllerAccessModalOpen() ? controllerAccessList : controllerClosePageList;
    const options = getControllerPageOptions(container);
    if (!options.length) {
        return true;
    }
    if (direction === 'up' || direction === 'left') {
        selectedControllerPageIndex -= 1;
    }
    else if (direction === 'down' || direction === 'right') {
        selectedControllerPageIndex += 1;
    }
    selectedControllerPageIndex = Math.max(0, Math.min(options.length - 1, selectedControllerPageIndex));
    updateControllerPageSelection(container);
    return true;
}
function createControllerPageOption({ id, title, subtitle, icon, type }, selected = false) {
    ...
    button.addEventListener('click', () => {
        const options = getControllerPageOptions(button.parentElement);
        selectedControllerPageIndex = Math.max(0, options.indexOf(button));
        updateControllerPageSelection(button.parentElement, false);
    });
    button.classList.toggle('is-controller-selected', selected);
    return button;
}
async function readControllerBrowserTabs() {
...
function renderControllerAccessOptions() {
    if (!controllerAccessList) {
        return;
    }
    controllerAccessList.innerHTML = '';
    const items = [
        {
            id: 'games',
            type: 'games',
        ...
        ...controllerBrowserTabs.map((tab) => ({
            id: tab.id,
            type: 'tab',
            title: tab.title || 'Pagina aberta',
            subtitle: tab.url || 'Rodando em segundo plano',
            icon: tab.active ? 'bi bi-window-check' : 'bi bi-window'
        }))
    ];
    items.forEach((item, index) => {
        controllerAccessList.appendChild(createControllerPageOption(item, index === selectedControllerPageIndex));
    });
    updateControllerPageSelection(controllerAccessList, false);
}
function renderControllerCloseOptions() {
    if (!controllerClosePageList) {
        return;
    }
    controllerClosePageList.innerHTML = '';
    if (!controllerBrowserTabs.length) {
        const empty = createControllerPageOption({
            id: '',
            type: 'empty',
            title: 'Nenhuma pagina aberta',
            subtitle: 'Abra YouTube, Google ou Discord pelo Web App',
            icon: 'bi bi-window-x'
        }, true);
        empty.disabled = true;
        controllerClosePageList.appendChild(empty);
        return;
    }
    controllerBrowserTabs.forEach((tab, index) => {
        controllerClosePageList.appendChild(createControllerPageOption({
            id: tab.id,
            type: 'tab',
            title: tab.title || 'Pagina aberta',
            subtitle: tab.url || 'Rodando em segundo plano',
            icon: tab.active ? 'bi bi-window-check' : 'bi bi-window'
        }, index === selectedControllerPageIndex));
    });
    updateControllerPageSelection(controllerClosePageList, false);
}
async function openControllerAccessModal() {
    if (introActive || profileSelectionActive || isVirtualKeyboardOpen()) {
        return;
    }
    controllerBrowserTabs = await readControllerBrowserTabs();
    selectedControllerPageIndex = 0;
    renderControllerAccessOptions();
    controllerAccessModal?.show();
}
async function openControllerClosePageModal() {
    if (introActive || profileSelectionActive || isVirtualKeyboardOpen()) {
        return;
    }
    controllerBrowserTabs = await readControllerBrowserTabs();
    selectedControllerPageIndex = 0;
    renderControllerCloseOptions();
    controllerClosePageModal?.show();
}
function openGameSearchModal() {
    if (introActive || profileSelectionActive || !gameSearchModalElement) {
        return;
    }
    buildVirtualKeyboard('search');
    keyboardShift = false;
    selectedKeyboardIndex = 0;
    useVirtualKeyboard('search');
    syncGameSearchInputs(gameSearchController.pattern);
    setActiveKeyboardInput(gameSearchModalInput);
...
function openControllerModalFromSessionFlag() {
    const modalType = sessionStorage.getItem('launcher-controller-modal');
    if (!modalType) {
        return;
    }
    if (introActive || profileSelectionActive) {
        window.setTimeout(openControllerModalFromSessionFlag, 250);
        return;
    }
    sessionStorage.removeItem('launcher-controller-modal');
    if (modalType === 'close') {
        openControllerClosePageModal().catch((error) => {
            showControllerAlert(`Erro ao listar paginas abertas: ${error}`);
        });
    }
    else {
        openControllerAccessModal().catch((error) => {
            showControllerAlert(`Erro ao listar paginas abertas: ${error}`);
        });
    }
}
function confirmControllerAccess() {
    const selected = getControllerPageOptions(controllerAccessList)[selectedControllerPageIndex];
    if (!selected) {
        return true;
    }
    controllerAccessModal?.hide();
    if (selected.dataset.type === 'games') {
        setNavigationArea('games', selectedGameIndex, false);
        return true;
    }
    if (selected.dataset.type === 'tab' && selected.dataset.id) {
        sessionStorage.setItem('launcher-open-tab-id', selected.dataset.id);
        window.location.href = `./webapp.html?tab=${encodeURIComponent(selected.dataset.id)}`;
    }
    return true;
}
async function confirmControllerClosePage() {
    const selected = getControllerPageOptions(controllerClosePageList)[selectedControllerPageIndex];
    if (!selected || selected.dataset.type !== 'tab' || !selected.dataset.id) {
        return true;
    }
    const response = await window.pywebview?.api?.fechar_url_launcher?.(selected.dataset.id);
    if (response?.ok === false) {
        showControllerAlert(response.message);
        return true;
    }
    controllerBrowserTabs = Array.isArray(response?.tabs) ? response.tabs : await readControllerBrowserTabs();
    selectedControllerPageIndex = Math.max(0, Math.min(selectedControllerPageIndex, controllerBrowserTabs.length - 1));
    if (!controllerBrowserTabs.length) {
        controllerClosePageModal?.hide();
        return true;
    }
    renderControllerCloseOptions();
    return true;
...
function setProfileModeLabels(mode) {
    ...
    if (profileSourceLabel)
        profileSourceLabel.textContent = labels.source || 'Perfil';
    if (profileStatOneLabel)
        profileStatOneLabel.textContent = labels.statOne || '';
    if (profileStatTwoLabel)
        profileStatTwoLabel.textContent = labels.statTwo || '';
    if (profileStatThreeLabel)
        profileStatThreeLabel.textContent = labels.statThree || '';
    if (profileDetailsTitle)
        profileDetailsTitle.textContent = labels.details || 'Detalhes';
    if (profileAsideTitle)
        profileAsideTitle.textContent = labels.aside || 'Recentes';
}
...
function setActiveKeyboardInput(input) {
    ...
    [steamGameName, steamAppId, steamApiKey, romDisplayNameInput, gameSearchModalInput].forEach((item) => {
...
function updateKeyboardLabels() {
    getKeyboardButtons().forEach((button) => {
        ...
        if (button.dataset.action === 'save') {
            button.textContent = activeKeyboardContext === 'search' ? 'Buscar' : 'Salvar';
        }
    });
}
function useVirtualKeyboard(context = 'steam') {
    activeKeyboardContext = context;
    currentVirtualKeyboard = context === 'rom'
        ? romRenameKeyboard
        : context === 'search'
            ? gameSearchKeyboard
            : addSteamVirtualKeyboard;
}
...
function isRomRenameKeyboardOpen() {
    return Boolean(trailerModalElement?.classList.contains('show')
        && selectedModalGame?.tipo === 'rom'
        && !romRenamePanel?.classList.contains('d-none'));
}
function isGameSearchKeyboardOpen() {
    return Boolean(gameSearchModalElement?.classList.contains('show'));
}
function isVirtualKeyboardOpen() {
    return Boolean(addSteamModalElement?.classList.contains('show') || isRomRenameKeyboardOpen() || isGameSearchKeyboardOpen());
}
...
function toggleKeyboardField() {
    const fields = activeKeyboardContext === 'rom'
        ...
        : activeKeyboardContext === 'search'
            ? [gameSearchModalInput].filter(Boolean)
...
function pressVirtualKeyboardKey(button = getKeyboardButtons()[selectedKeyboardIndex]) {
    ...
    if (action === 'field') {
        ...
        else if (activeKeyboardContext === 'search') {
            applyGameSearch(gameSearchModalInput?.value || '', true);
...
function setProtonTierBadgeStyle(tierBadge, tier) {
    ...
    tierBadge.classList.remove('proton-badge', 'platinum', 'gold', 'silver', 'bronze', 'borked', 'unknown', 'proton-tier-platinum', 'proton-tier-gold', 'proton-tier-silver', 'proton-tier-bronze', 'proton-tier-borked', 'proton-tier-pending');
...
function createSteamGameCard(game) {
    ...
    if (existingCard) {
        ...
        existingCard.dataset.gameSearchText = getGameSearchText(game, existingCard);
    ...
    card.dataset.gameSearchText = getGameSearchText(game, card);
    card.dataset.protonTier = 'unknown';
...
async function loadSteamGames() {
    ...
    try {
        const games = await withUiTimeout(window.pywebview.api.listar_jogos(), 12000, 'O carregamento da biblioteca demorou demais.');
        games
            .filter((game) => game.tipo === 'steam' ||
...
function launcherNavigate(direction) {
    ...
    if (isControllerAccessModalOpen() || isControllerClosePageModalOpen()) {
        navigateControllerPageModal(direction);
        return;
    }
    if (introActive) {
        return;
    ...
    if (navigationArea === 'navbar') {
        ...
        if (direction === 'left') {
            ...
            setNavigationArea(getSearchItems().length ? 'search' : 'categories', getSearchItems().length ? 0 : selectedCategoryIndex);
            return;
        }
    ...
    if (navigationArea === 'search') {
        ...
        if (direction === 'left' || direction === 'right') {
            gameSearchInput?.focus({ preventScroll: true });
        }
        updateNavigationSelection(false);
        return;
    }
    if (navigationArea === 'categories') {
        ...
        if (direction === 'left') {
            ...
            setNavigationArea(getSearchItems().length ? 'search' : 'navbar', getSearchItems().length ? 0 : getAddGameNavbarIndex());
            return;
        }
...
function launcherModalAction(action) {
    ...
    if (window.PageTabsModal?.handleAction?.(action)) {
        return true;
    }
    if (isControllerAccessModalOpen()) {
        if (action === 'confirm') {
            confirmControllerAccess();
            return true;
        }
        if (action === 'back') {
            controllerAccessModal?.hide();
    ...
    if (isControllerClosePageModalOpen()) {
        if (action === 'confirm') {
            confirmControllerClosePage().catch((error) => {
                showControllerAlert(`Erro ao fechar pagina: ${error}`);
            });
            return true;
        }
        if (action === 'back') {
...
updateNavigationSelection(false);
window.setTimeout(openControllerModalFromSessionFlag, 120);
...
controllerAccessModalElement?.addEventListener('shown.bs.modal', () => {
    updateControllerPageSelection(controllerAccessList, false);
});
controllerAccessModalElement?.addEventListener('hidden.bs.modal', () => {
    selectedControllerPageIndex = 0;
});
controllerClosePageModalElement?.addEventListener('shown.bs.modal', () => {
    updateControllerPageSelection(controllerClosePageList, false);
});
controllerClosePageModalElement?.addEventListener('hidden.bs.modal', () => {
    selectedControllerPageIndex = 0;
});
controllerAccessConfirm?.addEventListener('click', confirmControllerAccess);
controllerAccessCancel?.addEventListener('click', () => controllerAccessModal?.hide());
controllerClosePageConfirm?.addEventListener('click', () => {
    confirmControllerClosePage().catch((error) => {
        showControllerAlert(`Erro ao fechar pagina: ${error}`);
    });
});
controllerClosePageCancel?.addEventListener('click', () => controllerClosePageModal?.hide());
document.addEventListener('keydown', (event) => {
    if (!isControllerAccessModalOpen() && !isControllerClosePageModalOpen()) {
        return;
    }
    if (event.key === 'Enter') {
        event.preventDefault();
        launcherModalAction('confirm');
    }
    else if (event.key === 'Escape') {
        event.preventDefault();
        launcherModalAction('back');
    }
}, true);
        ...
        if (gamepad) {
            ...
            if (window.PageTabsModal?.isOpen?.()) {
                if (direction && (direction !== previousDirection || now - lastMove > 180)) {
                    runControllerAction(() => window.PageTabsModal.navigate(direction));
                    lastMove = now;
                }
                if (pressedOnce(gamepad, 0)) {
                    runControllerAction(() => window.PageTabsModal.handleAction('confirm'));
                }
                if (pressedOnce(gamepad, 1)) {
                    runControllerAction(() => window.PageTabsModal.handleAction('back'));
                }
                previousDirection = direction;
                previousButtons = gamepad.buttons.map((button) => button.pressed);
                requestAnimationFrame(loop);
            ...
            if (isControllerAccessModalOpen() || isControllerClosePageModalOpen()) {
                if (direction && (direction !== previousDirection || now - lastMove > 180)) {
                    runControllerAction(() => navigateControllerPageModal(direction));
                    lastMove = now;
                }
                if (pressedOnce(gamepad, 0)) {
                    runControllerAction(() => launcherModalAction('confirm'));
                }
                if (pressedOnce(gamepad, 1)) {
                    runControllerAction(() => launcherModalAction('back'));
                }
                previousDirection = direction;
                previousButtons = gamepad.buttons.map((button) => button.pressed);
                requestAnimationFrame(loop);
            ...
            if (!keyboardOpen && pressedOnce(gamepad, 9)) {
                runControllerAction(() => window.PageTabsModal?.openAccess?.().catch((error) => {
                    showControllerAlert(`Erro ao listar paginas abertas: ${error}`);
                }));
            }
            if (!keyboardOpen && navigationArea === 'search' && pressedOnce(gamepad, 2)) {
                runControllerAction(openGameSearchModal);
                previousDirection = direction;
                previousButtons = gamepad.buttons.map((button) => button.pressed);
                requestAnimationFrame(loop);
                return;
            }
            if (!keyboardOpen && pressedOnce(gamepad, 8)) {
                runControllerAction(() => window.PageTabsModal?.openClose?.().catch((error) => {
                    showControllerAlert(`Erro ao listar paginas abertas: ${error}`);
                }));
            }
            if (pressedOnce(gamepad, 0)) {

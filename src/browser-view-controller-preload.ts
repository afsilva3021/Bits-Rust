...
function readDirection(gamepad) {
  ...
  if (gamepad.buttons[12]?.pressed || axisY < -0.65) return 'up';
  if (gamepad.buttons[13]?.pressed || axisY > 0.65) return 'down';
  if (gamepad.buttons[14]?.pressed || axisX < -0.65) return 'left';
  if (gamepad.buttons[15]?.pressed || axisX > 0.65) return 'right';
...
function loop() {
  ...
  if (gamepad) {
    ...
    if (keyboardMode) {
      ...
      if (pressedOnce(gamepad, 0)) sendAction('keyboard-press');
      if (pressedOnce(gamepad, 1)) {
        keyboardMode = false;
    ...
    if (pressedOnce(gamepad, 0)) invokeInput('py:clicar_mouse_sistema');
    if (pressedOnce(gamepad, 6)) invokeInput('py:clicar_mouse_direito_sistema');
    if (pressedOnce(gamepad, 7)) invokeInput('py:clicar_mouse_sistema');
    if (pressedOnce(gamepad, 1)) sendAction('back');
    if (pressedOnce(gamepad, 2)) {
      keyboardMode = true;
      sendAction('keyboard-open');
    }
    if (pressedOnce(gamepad, 3)) sendAction('reload');
    if (pressedOnce(gamepad, 8)) {
      sendAction('close-modal');
    }
    if (pressedOnce(gamepad, 9)) {

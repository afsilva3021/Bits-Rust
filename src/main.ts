...
async function resetLauncherBrowserVolatileStorage() {
  ...
  await Promise.allSettled(
    LAUNCHER_BROWSER_CACHE_DIRS.map((directory) => (
      fs.rm(path.join(partitionPath, directory), { recursive: true, force: true })
    ))
  );
}
...
function updateVirtualKeyboardWindow(state = {}, bounds = {}) {
  ...
  const sendState = () => {
    ...
    keyboardWindow.webContents.executeJavaScript(
      `window.updateKeyboardState(${JSON.stringify(state)})`,
      true
    ).catch(() => {});
  };
...
ipcMain.handle('py:ocultar_url_launcher', () => (
  hideActiveLauncherBrowserView()

...
let mainWindow;
const launcherBrowserTabs = new Map();
let activeLauncherBrowserTabId = '';
let launcherBrowserPoint = { x: 0, y: 0 };
let launcherBrowserCurrentUrl = '';
let discordNotificationWindow = null;
let virtualBrowserCursorWindow = null;
let virtualKeyboardWindow = null;
let launcherFocusTimers = [];
...
const LAUNCHER_BROWSER_PARTITION = 'persist:launcher-browser';
const LAUNCHER_BROWSER_CACHE_DIRS = [
    'Service Worker',
    'Cache',
    'Code Cache',
    'GPUCache',
...
function getLauncherBrowserSession() {
    return session.fromPartition(LAUNCHER_BROWSER_PARTITION, { cache: true });
}
function getLauncherBrowserPartitionPath() {
    const partitionName = LAUNCHER_BROWSER_PARTITION.replace(/^persist:/, '');
    return path.join(app.getPath('userData'), 'Partitions', partitionName);
}
async function resetLauncherBrowserVolatileStorage() {
    const partitionPath = getLauncherBrowserPartitionPath();
    await Promise.allSettled(LAUNCHER_BROWSER_CACHE_DIRS.map((directory) => (fs.rm(path.join(partitionPath, directory), { recursive: true, force: true }))));
}
async function healLauncherBrowserSessionStorage(browserSession) {
    await resetLauncherBrowserVolatileStorage();
    try {
        ...
        console.warn('Falha ao limpar cache do navegador interno:', error.message);
    }
    try {
        await browserSession.clearCache();
    }
    catch (error) {
        console.warn('Falha ao limpar cache HTTP do navegador interno:', error.message);
    }
}
function configureLauncherBrowserSession(browserSession) {
...
function sortGamesByName(games = []) {
    return [...games].sort((a, b) => String(a.nome || a.title || '')
...
async function listRoms(data) {
    ...
    const romGroups = await Promise.all(emulators.map(async (emulator) => {
        const roms = await withTimeout(scanDirectoryForRoms(emulator), 5000, `Tempo esgotado ao escanear ROMs de ${emulator.nome || emulator.id}`).catch((error) => {
...
async function listAllUiGames() {
    ...
    const enrichedSavedGames = await Promise.all(savedGames.map((game) => enrichSteamInstallStatus(game, installedSteamAppIds)));
    return [
        ...enrichedSavedGames,
        ...roms.map(toUiGame)
    ];
...
async function fetchSteamGridDbCover(appId) {
    ...
    const payload = await fetchJsonWithHeaders(`https://www.steamgriddb.com/api/v2/grids/steam/${cleanAppId}`, { Authorization: `Bearer ${key}` });
...
function createWindow() {
    ...
    mainWindow.on('closed', () => {
        destroyAllLauncherBrowserViews();
        mainWindow = null;
    });
...
function ensureVirtualBrowserCursorWindow() {
    if (virtualBrowserCursorWindow && !virtualBrowserCursorWindow.isDestroyed()) {
        return virtualBrowserCursorWindow;
    }
    virtualBrowserCursorWindow = new BrowserWindow({
        width: 36,
        height: 36,
        frame: false,
        transparent: true,
    ...
    virtualBrowserCursorWindow.setAlwaysOnTop(true, LAUNCHER_ALWAYS_ON_TOP_LEVEL);
    virtualBrowserCursorWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true });
    virtualBrowserCursorWindow.setIgnoreMouseEvents(true);
    const html = `
<!doctype html>
<html>
<head>
...
<body><div class="cursor"></div></body>
    ...
    virtualBrowserCursorWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
    virtualBrowserCursorWindow.on('closed', () => {
        virtualBrowserCursorWindow = null;
    });
    return virtualBrowserCursorWindow;
}
function hideVirtualBrowserCursor() {
    if (virtualBrowserCursorWindow && !virtualBrowserCursorWindow.isDestroyed()) {
        virtualBrowserCursorWindow.hide();
    }
}
function moveVirtualBrowserCursor(bounds, point) {
    if (!mainWindow || mainWindow.isDestroyed()) {
        hideVirtualBrowserCursor();
        return;
    }
    const cursorWindow = ensureVirtualBrowserCursorWindow();
    const contentBounds = mainWindow.getContentBounds();
    const x = Math.round(contentBounds.x + bounds.x + point.x - 4);
    const y = Math.round(contentBounds.y + bounds.y + point.y - 4);
    cursorWindow.setBounds({ x, y, width: 36, height: 36 }, false);
    if (!cursorWindow.isVisible()) {
        cursorWindow.showInactive();
    }
}
function ensureVirtualKeyboardWindow(bounds = {}) {
    if (virtualKeyboardWindow && !virtualKeyboardWindow.isDestroyed()) {
        return virtualKeyboardWindow;
    }
    const display = screen.getPrimaryDisplay();
    const area = display.workArea;
    const width = Math.max(760, Math.min(area.width, Math.round(Number(bounds.width) || area.width)));
    const height = 390;
    const x = Math.round(area.x + (area.width - width) / 2);
    const y = Math.round(area.y + area.height - height);
    virtualKeyboardWindow = new BrowserWindow({
        width,
        height,
        x,
        y,
    ...
    virtualKeyboardWindow.setAlwaysOnTop(true, LAUNCHER_ALWAYS_ON_TOP_LEVEL);
    virtualKeyboardWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true });
    virtualKeyboardWindow.setIgnoreMouseEvents(true);
    virtualKeyboardWindow.loadFile(path.join(ROOT_DIR, 'interface', 'virtual-keyboard.html'));
    virtualKeyboardWindow.on('closed', () => {
        virtualKeyboardWindow = null;
    });
    return virtualKeyboardWindow;
}
function positionVirtualKeyboard(bounds = {}) {
    if (!virtualKeyboardWindow || virtualKeyboardWindow.isDestroyed()) {
        return;
    }
    const contentBounds = mainWindow?.getContentBounds();
    ...
    const width = Math.max(640, Math.round(Number(bounds.width) || contentBounds.width));
    const height = 390;
    const x = Math.round(contentBounds.x + (Number(bounds.x) || 0));
    const y = Math.round(contentBounds.y + (Number(bounds.y) || 0) + Math.max(0, (Number(bounds.height) || contentBounds.height) - height));
    virtualKeyboardWindow.setBounds({ x, y, width, height }, false);
}
function updateVirtualKeyboardWindow(state = {}, bounds = {}) {
    const keyboardWindow = ensureVirtualKeyboardWindow(bounds);
    positionVirtualKeyboard(bounds);
    const sendState = () => {
        if (keyboardWindow.isDestroyed()) {
            return;
        }
        keyboardWindow.webContents.executeJavaScript(`window.updateKeyboardState(${JSON.stringify(state)})`, true).catch(() => { });
    };
    if (keyboardWindow.webContents.isLoading()) {
        keyboardWindow.webContents.once('did-finish-load', sendState);
    }
    else {
        sendState();
    }
...
function hideVirtualKeyboardWindow() {
    if (virtualKeyboardWindow && !virtualKeyboardWindow.isDestroyed()) {
        virtualKeyboardWindow.hide();
    }
}
...
function getActiveLauncherBrowserTab() {
    return launcherBrowserTabs.get(activeLauncherBrowserTabId) || null;
}
function getActiveLauncherBrowserView() {
    return getActiveLauncherBrowserTab()?.view || null;
}
function sendNativeBrowserControllerAction(action, payload = {}) {
    if (!mainWindow || mainWindow.isDestroyed()) {
        return;
    }
    mainWindow.webContents.send('launcher:native-browser-controller-action', action, payload);
}
function focusLauncherControlRenderer() {
    if (!mainWindow || mainWindow.isDestroyed()) {
        return;
    }
    launcherFocusTimers.forEach((timer) => clearTimeout(timer));
    launcherFocusTimers = [];
    const focus = () => {
        if (!mainWindow || mainWindow.isDestroyed()) {
            return;
        }
        const activeView = getActiveLauncherBrowserView();
        if (activeView && !activeView.webContents.isDestroyed()) {
            try {
                activeView.webContents.blur();
            }
            catch {
                // Some Electron builds do not expose blur for BrowserView contents.
            }
        }
        mainWindow.focus();
    ...
    [0, 50, 150, 350, 700, 1200, 2000].forEach((delay) => {
        launcherFocusTimers.push(setTimeout(focus, delay));
    });
}
function serializeLauncherBrowserTab(tab) {
    return {
        id: tab.id,
        pageKey: tab.pageKey || getLauncherBrowserPageKey(tab.url),
        title: tab.title || 'Navegador',
        url: tab.url || '',
        active: tab.id === activeLauncherBrowserTabId
    };
}
function listLauncherBrowserTabs() {
    compactLauncherBrowserTabs();
    return Array.from(launcherBrowserTabs.values()).map(serializeLauncherBrowserTab);
}
function getLauncherBrowserPageKey(url) {
    try {
...
function getLauncherBrowserTabIdentity(url) {
    return getLauncherBrowserPageKey(url) || normalizeWebUrl(url);
}
function findLauncherBrowserTabByUrl(url) {
    const identity = getLauncherBrowserTabIdentity(url);
    return Array.from(launcherBrowserTabs.values()).find((tab) => {
        const tabIdentity = tab.pageKey || getLauncherBrowserTabIdentity(tab.url);
        return tabIdentity === identity;
    }) || null;
}
function compactLauncherBrowserTabs() {
    const seen = new Map();
    for (const tab of Array.from(launcherBrowserTabs.values()).reverse()) {
        const identity = tab.pageKey || getLauncherBrowserTabIdentity(tab.url);
        if (!identity) {
            continue;
        }
        if (!seen.has(identity)) {
            tab.pageKey = getLauncherBrowserPageKey(tab.url);
            seen.set(identity, tab.id);
            continue;
        }
        destroyLauncherBrowserView(tab.id);
    }
}
function attachLauncherBrowserTab(tab, bounds = tab.bounds) {
    if (!mainWindow || mainWindow.isDestroyed() || !tab) {
        return null;
    }
    for (const item of launcherBrowserTabs.values()) {
        if (item.id !== tab.id) {
            try {
                mainWindow.removeBrowserView(item.view);
            }
            catch {
                // The view may already be detached while the page keeps running.
            }
        }
    }
    try {
        ...
        // addBrowserView throws if Electron thinks the same view is already attached.
    }
    activeLauncherBrowserTabId = tab.id;
    launcherBrowserCurrentUrl = tab.url || '';
    const normalizedBounds = setLauncherBrowserBounds(bounds);
    focusLauncherControlRenderer();
    return normalizedBounds;
}
function createLauncherBrowserTab(url, title) {
    const id = createId('webtab');
    const view = new BrowserView({
        webPreferences: {
            preload: path.join(__dirname, 'browser-view-controller-preload.js'),
            contextIsolation: true,
            nodeIntegration: false,
            sandbox: true,
            backgroundThrottling: false,
            partition: LAUNCHER_BROWSER_PARTITION
        }
    });
    const tab = {
        id,
        pageKey: getLauncherBrowserPageKey(url),
    ...
    view.webContents.on('did-navigate', (_event, url) => {
        tab.url = url;
        tab.pageKey = getLauncherBrowserPageKey(url);
        if (tab.id === activeLauncherBrowserTabId) {
    ...
    launcherBrowserTabs.set(id, tab);
    return tab;
}
function destroyLauncherBrowserView(tabId = activeLauncherBrowserTabId) {
    const tab = launcherBrowserTabs.get(tabId);
    if (!tab) {
        return;
    }
    if (mainWindow && !mainWindow.isDestroyed()) {
    ...
    launcherBrowserTabs.delete(tab.id);
    if (activeLauncherBrowserTabId === tab.id) {
        activeLauncherBrowserTabId = launcherBrowserTabs.keys().next().value || '';
    }
    launcherBrowserPoint = { x: 0, y: 0 };
    launcherBrowserCurrentUrl = getActiveLauncherBrowserTab()?.url || '';
    if (!activeLauncherBrowserTabId) {
        hideVirtualBrowserCursor();
        hideVirtualKeyboardWindow();
    }
    lastDiscordUnreadCount = 0;
}
function destroyAllLauncherBrowserViews() {
    for (const tabId of Array.from(launcherBrowserTabs.keys())) {
        destroyLauncherBrowserView(tabId);
    }
    hideVirtualBrowserCursor();
    hideVirtualKeyboardWindow();
}
function hideActiveLauncherBrowserView() {
    const tab = getActiveLauncherBrowserTab();
...
function setLauncherBrowserBounds(bounds) {
    const launcherBrowserView = getActiveLauncherBrowserView();
    const tab = getActiveLauncherBrowserTab();
    if (!launcherBrowserView || !tab) {
        return null;
    }
...
function sendBrowserMouseMove(x, y) {
    ...
    moveVirtualBrowserCursor(bounds, launcherBrowserPoint);
    focusLauncherControlRenderer();
...
async function launchItem(item) {
    ...
    try {
        if (item.type === 'lutris') {
            ...
            const result = await launchAndCheck('lutris', [`lutris:rungameid/${item.lutrisId}`], monitorToken);
            if (!result.ok) {
                finishGameMonitor(monitorToken);
            }
...
ipcMain.handle('py:adicionar_jogo_steam', async (_event, nome, appIdOuCaminho, categoria) => (addOrEditGame({ nome, appIdOuCaminho, categoria })));
ipcMain.handle('py:editar_jogo_adicionado', async (_event, id, nome, appIdOuCaminho, categoria) => (addOrEditGame({ id, nome, appIdOuCaminho, categoria })));
...
ipcMain.handle('py:adicionar_jogo_lutris', async (_event, nome, lutrisId, categoria) => (addOrEditLutrisGame({ nome, lutrisId, categoria })));
ipcMain.handle('py:editar_jogo_lutris', async (_event, id, nome, lutrisId, categoria) => (addOrEditLutrisGame({ id, nome, lutrisId, categoria })));
...
ipcMain.handle('py:importar_biblioteca_steam', async () => {
    ...
    try {
        ...
        const jogos = await Promise.all([...imported, ...updated].map((game) => enrichSteamInstallStatus(game, installedSteamAppIds)));
...
ipcMain.handle('py:abrir_url_launcher', async (_event, url, _title, bounds) => {
    try {
        ...
        const title = String(_title || 'Navegador').trim() || 'Navegador';
        const tab = findLauncherBrowserTabByUrl(normalizedUrl) || createLauncherBrowserTab(normalizedUrl, title);
        const isNewTab = !tab.view.webContents.getURL();
        const normalizedBounds = attachLauncherBrowserTab(tab, bounds);
        if (!normalizedBounds) {
            return fail('Janela principal indisponivel.');
        }
        tab.title = title;
        tab.url = normalizedUrl;
        tab.pageKey = getLauncherBrowserPageKey(normalizedUrl);
        ...
        return ok('URL aberta no navegador interno do launcher.', {
            bounds: normalizedBounds,
            tab: serializeLauncherBrowserTab(tab),
            tabs: listLauncherBrowserTabs()
        });
    }
    catch (error) {
        return fail(error.message);
...
ipcMain.handle('py:listar_url_launcher', () => ok('Abas listadas.', { tabs: listLauncherBrowserTabs() }));
ipcMain.handle('py:registrar_url_launcher', async (_event, url, _title) => {
    try {
        const normalizedUrl = normalizeWebUrl(url);
        const title = String(_title || 'Navegador').trim() || 'Navegador';
        const tab = findLauncherBrowserTabByUrl(normalizedUrl) || createLauncherBrowserTab(normalizedUrl, title);
        ...
        return ok('Pagina registrada em segundo plano.', {
            tab: serializeLauncherBrowserTab(tab),
            tabs: listLauncherBrowserTabs()
        });
    }
...
ipcMain.handle('py:ativar_url_launcher', (_event, tabId, bounds) => {
    const tab = launcherBrowserTabs.get(String(tabId || ''));
    if (!tab) {
        return fail('Pagina nao encontrada.');
    }
    const normalizedBounds = attachLauncherBrowserTab(tab, bounds);
    if (!normalizedBounds) {
        return fail('Janela principal indisponivel.');
    ...
    sendBrowserMouseMove(launcherBrowserPoint.x, launcherBrowserPoint.y);
    return ok('Pagina ativada.', {
        bounds: normalizedBounds,
        tab: serializeLauncherBrowserTab(tab),
        tabs: listLauncherBrowserTabs()
    });
});
ipcMain.handle('py:ocultar_url_launcher', () => (hideActiveLauncherBrowserView()
    ? ok('Navegador interno oculto.')
    : fail('Nenhuma pagina aberta no launcher.')));
ipcMain.handle('py:fechar_url_launcher', (_event, tabId) => {
    if (tabId) {
        destroyLauncherBrowserView(String(tabId));
    }
    else {
        destroyLauncherBrowserView();
    }
    return ok('Pagina fechada.', { tabs: listLauncherBrowserTabs() });
});
...
ipcMain.handle('py:mostrar_teclado_virtual', (_event, state, bounds) => {
    updateVirtualKeyboardWindow(state, bounds);
    return ok('Teclado virtual exibido.');
});
ipcMain.handle('py:atualizar_teclado_virtual', (_event, state, bounds) => {
    updateVirtualKeyboardWindow(state, bounds);
    return ok('Teclado virtual atualizado.');
});
ipcMain.handle('py:fechar_teclado_virtual', () => {
    hideVirtualKeyboardWindow();
    return ok('Teclado virtual fechado.');
});
...
ipcMain.handle('py:voltar_url_launcher', () => {
    ...
    const { webContents } = launcherBrowserView;
    const navigationHistory = webContents.navigationHistory;
    const canGoBack = navigationHistory && typeof navigationHistory.canGoBack === 'function'
        ? navigationHistory.canGoBack()
        : webContents.canGoBack();
    if (canGoBack) {
        if (navigationHistory && typeof navigationHistory.goBack === 'function') {
...
ipcMain.on('launcher:native-browser-controller-action', (_event, action, payload = {}) => {
    const normalizedAction = String(action || '');
    if (normalizedAction === 'access-modal' || normalizedAction === 'close-modal') {
        hideActiveLauncherBrowserView();
    }
    sendNativeBrowserControllerAction(normalizedAction, payload);
});
ipcMain.on('launcher:native-browser-keyboard-mode', (_event, open) => {
    const launcherBrowserView = getActiveLauncherBrowserView();
    if (!launcherBrowserView || launcherBrowserView.webContents.isDestroyed()) {
        return;
    }
    launcherBrowserView.webContents.send('launcher:native-browser-keyboard-mode', Boolean(open));
});
ipcMain.on('launcher:native-browser-modal-mode', (_event, open) => {
    const launcherBrowserView = getActiveLauncherBrowserView();
    if (!launcherBrowserView || launcherBrowserView.webContents.isDestroyed()) {
        return;
    }
    launcherBrowserView.webContents.send('launcher:native-browser-modal-mode', Boolean(open));
});
app.whenReady().then(async () => {
    const launcherBrowserSession = getLauncherBrowserSession();
    await healLauncherBrowserSessionStorage(launcherBrowserSession);
    configureLauncherBrowserSession(launcherBrowserSession);
